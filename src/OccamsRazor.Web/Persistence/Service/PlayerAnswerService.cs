...
namespace OccamsRazor.Web.Persistence.Service
{
    ...
    {
        private readonly IPlayerAnswerRepository playerAnswerRepository;
        ...
        public Task<bool> SubmitPlayerAnswer(PlayerAnswer answer)
            => playerAnswerRepository.SetPlayerAnswerAsync(answer);

        public Task<IEnumerable<PlayerAnswer>> Get(int gameId) => playerAnswerRepository.GetAllAnswers(gameId);
        public async Task<IEnumerable<GameResults>> GetScoresForGame(int gameId)
        {
            var answers = await playerAnswerRepository.GetAnswersAsync(gameId);
